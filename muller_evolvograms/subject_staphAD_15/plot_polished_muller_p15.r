# Rscript to generate polished muller plot for subject 15
library(ggmuller)
library(ggplot2)
...
pdf('ggmuller_plot_pat15.pdf', width=14, height=4.5, onefile = TRUE) 
my_palette <- c('white','#f03b20','#feb24c')
mc=my_palette
names(mc)=c('genotype-0','genotype-1','genotype-2')
ggplot(muller_df, aes_string(x = "Generation", y = "Frequency", group = "Group_id", fill = "Identity", colour = "Identity")) + 
    ...
    scale_x_continuous(breaks = c(0,200,300,1300),labels=c("0\n12","2\n49","3\n16","13\n8"), name="Time (in months)",limits = c(0, 1400)) +
